...
test('homepage shows hero and navigation', async ({ page }) => {
  ...
  // ACTUALLY TEST CLICKING AND NAVIGATION
  const projectsLink = nav.getByRole('link', { name: /projects/i });
  await projectsLink.click();  // ← Real click in real browser!
  await expect(page).toHaveURL(/\/projects/);  // ← Verify navigation worked
...
- ✅ **Does clicking actually work?** (real browser interaction)
- ✅ **Does navigation work?** (URL changes, page loads)
